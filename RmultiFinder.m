function [RmultiMat] = RmultiFinder(Rtot, Kd, mfiAdjMean, biCoefMat, tnpbsa)
    ...
    for j = 1:6
        for k = 1:4
            RmultiMat(j,k) = RmultiSolver(Rtot(j),Kx,v(1),Kd(j,k), ...
                tnpbsa(1),biCoefMat);
            RmultiMat(j,k+4) = RmultiSolver(Rtot(j),Kx,v(2),Kd(j,k), ...
                tnpbsa(2),biCoefMat);
        end
    end
end
%--------------------------------------------------------------------------
function Rmulti = RmultiSolver(Rtot,Kx,v,Kd,L0,biCoefMat)    
