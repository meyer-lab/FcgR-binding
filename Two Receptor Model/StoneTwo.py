...
def nmultichoosek(n, k, i):
    return comb(n, k, exact=True)*comb(k, i, exact=True)
...
def StoneVgrid(Req,Ka,gnu,Kx,L0):
    ...
    vGrid = np.empty([gnu+1, gnu+1], dtype = np.float64)

    # ii is the total number of receptors bound
    for ii in range(gnu+1):
        # jj is the number of receptor one bound
        for jj in range(ii+1):
            vGrid[ii, jj] = L0*Ka*nmultichoosek(gnu,ii,jj)*Kx**ii*Req[0]**(jj)*Req[1]**(ii-jj)
