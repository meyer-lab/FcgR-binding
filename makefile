...
test:
	nosetests-3 -s --with-timer --timer-top-n 5

profile:
	nosetests-3 -s --with-timer --timer-top-n 5 --with-cprofile
	gprof2dot -n 2.0 -e 2.0 -f pstats stats.dat | dot -Tpng -o profile.png

testcover:
	nosetests-3 --with-xunit --with-xcoverage --cover-package=recepmod -s --with-timer --timer-top-n 5
