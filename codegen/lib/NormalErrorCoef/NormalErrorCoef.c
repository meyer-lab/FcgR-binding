...
#include "StoneMod.h"
#include "power.h"
#include "NormalErrorCoef_rtwutil.h"
 ...
 * Return Type  : double
 */
double NormalErrorCoef(const double Rtot[12], const double KdMat[60], const
    ...
    for (ixstart = 0; ixstart < 6; ixstart++) {
      for (l = 0; l < 4; l++) {
        MFI = c * StoneMod(Rtot[ixstart], KdMat[ixstart + 6 * l], Rtot[9 + j],
                           log10(KdMat[ixstart + 6 * l] * rt_powd_snf(10.0,
...
/*
 * File trailer for NormalErrorCoef.c
 *
 * [EOF]
 */
