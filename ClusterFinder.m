function [ClusterMat] = ClusterFinder(Rtot, Kd, mfiAdjMean, biCoefMat, tnpbsa)
    ...
    ClusterMat = zeros(6,8);
    for j = 1:6
        for k = 1:4
            ClusterMat(j,k) = ClusterSolver(Rtot(j),Kx,v(1),Kd(j,k), ...
                tnpbsa(1),biCoefMat);
            ClusterMat(j,k+4) = ClusterSolver(Rtot(j),Kx,v(2),Kd(j,k), ...
                tnpbsa(2),biCoefMat);
...
%--------------------------------------------------------------------------
function L = ClusterSolver(Rtot,Kx,v,Kd,L0,biCoefMat)    
    ...
    v = v;
    L = sum(L0/Kd*(biCoefMat(2:v,v)'.*(Kx.^([2:v]-1))).*Req.^[2:v]);
end
%--------------------------------------------------------------------------
