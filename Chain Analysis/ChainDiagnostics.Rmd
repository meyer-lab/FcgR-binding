...
rm(bestFit, normData, FcgR.Ka, FcgR.quant)

data <- as.data.frame(data)
data <- mutate(data, crawler = as.factor(V2), LL = V1, gnu1 = floor(V6), gnu2 = floor(V7), sigma = V8, Kx = V3) %>%
  select(-V2, -V1, -V6, -V7, -V8, -V3) %>% filter(LL > max(LL)-2) %>% unique
```

```{r}
ggplot(allFit, aes(x = Fit, y = Meas, color = FcgR, shape = TNP)) + geom_point() + 
  annotate("segment", x = 0.1, xend = 4, y = 0.1, yend = 4, colour = "black") + scale_x_log10()
```
...
```{r}
ggplot(data, aes(x = sigma, y = LL, color = factor(gnu2), shape = factor(gnu1))) + geom_point()
```
